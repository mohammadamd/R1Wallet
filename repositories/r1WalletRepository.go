...
const (
	getBalanceQuery        = "SELECT current_balance FROM user_transactions WHERE user_id = ? ORDER BY id DESC LIMIT 1"
	insertTransactionQuery = "INSERT INTO user_transactions (user_id, amount, current_balance, description) " +
		"VALUES (?, ?, ?, ?)"
)
